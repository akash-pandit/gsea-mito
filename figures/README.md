...
- `top-3-terms-NES.pdf`: multi-GSEA, picks top 3 terms (per DEG sample) by Normalized Enrichment Score, ranked by absval(NES) (NES magnitude). Uses `MSigDB_Hallmark_2020` as the pathway library which is general, not mito-specific.
- `top-5-terms-NES.pdf`: multi-GSEA, like the `top-3` figure but uses `Mouse.MitoCarta3.0` as the pathway library, which only includes mitochondria-specific pathways.
